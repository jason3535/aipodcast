...
# 2026-08-14 扩充:传记式自述不一定带公司名(用户报的 fadell-lexfridm sec7[6]:
# 「在大学里我是一名 DJ/毕业后我搬到硅谷/我的右耳听力受损」全部漏检)。
# 补不锚定公司的人生叙事形态;主持人聊观点常用「我认为/我觉得」,不会用这些句式。
INSIDER = re.compile(r'(史蒂夫|乔布斯|马斯克|黄仁勋|山姆)说[:：“"]|我以前(构建|做|开发)过|我们(当时)?(决定|发布|做出|推翻)|我(领导|负责|设计)了')
FP_GUEST = re.compile(r'我(在|加入|离开|创办|创立) ?(Apple|苹果|谷歌|Google|OpenAI|Anthropic|DeepMind|微软|Meta|特斯拉|Nest|英特尔|NVIDIA|英伟达)|乔布斯(对|跟|告诉)我|我发明|我们发布了|我带的团队'
                      r'|毕业后我|在大学里?，?我|我(搬到|退学|从小就)|我是一名 ?[A-Za-z一-鿿]{1,6}(师|员|DJ|生)|我的(右耳|童年|父母)')
    ...
    for si, s in enumerate(d.get('transcript', [])):
        ...
        for ti, t in enumerate(turns):
            ...
            if spk != 'Host':
                ...
                # 亲历者叙事:Host 长轮里出现「名人直接引语 + 内部决策动作」≥2 处 →
                # 多半是嘉宾的整段自述被整轮标错(fadell sec17[4] 全轮 iPhone 键盘/SIM 卡槽内幕
                # 就是这么漏的:不带公司名、也没有「我在X」句式,FP_GUEST 抓不到)。
                if len(zh) > 320 and len(INSIDER.findall(zh)) >= 2:
                    hits += 1
                    print(f"  ⚠️ 疑似错标[Host 讲亲历内幕] {eid} sec{si}[{ti}] ({s.get('sec','')[:30]}): {zh[:70]}")
