...
请据此提炼这个人的【观点演变】:他的思考随时间如何变化、有哪些一以贯之的主线、哪些立场发生了转变或深化。3-5 条,按时间脉络组织。
- 每条尽量点出时间/变化(如"早期…到 2026 年…"或"始终主张…");只基于给定材料,不杜撰。
- 每条给 en(≤26 词)+ 地道中文 zh。严格用术语表。只输出 JSON:{{"views":[{{"en":"...","zh":"..."}}]}}
术语表:
{GT}"""
...
def load_eps():
    ...
    return json.loads(h[a+len("const EPISODES = "):b].rstrip().rstrip(";").rstrip())
